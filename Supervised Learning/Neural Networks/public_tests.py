...
def shape_as_list(shape):
    # Keras 2 returned a TensorShape (with .as_list()); Keras 3 returns a tuple.
...
def test_c1(target):
    ...
    assert shape_as_list(target.input_shape) == [None, 400], \
        f"Wrong input shape. Expected [None,  400] but got {shape_as_list(target.input_shape)}"
    i = 0
